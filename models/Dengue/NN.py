...
class Dengue_NN:
    def __init__(
        ...
        **__,
        ...
        # Training data
        self.training_data_org = training_data
        self.training_data = self.training_data_org.log1p()  # Log normalize training data
        ...
        if num_epochs > 0:
            self.lr_scheduler = optim.lr_scheduler.OneCycleLR(
                self.neural_net.optimizer,
                max_lr=self.neural_net.learning_rate,
                div_factor=20,  # Determines initial lr via initial_lr = max_lr/div_factor
    ...
    def rescale_parameters(self, predicted_parameters, ranges):
        """
        Scale predicted parameters tensor to their specific ranges using linear scaling
        and clipping to ensure values stay within valid ranges.
        ...
        Returns:
            torch.Tensor: Scaled and clipped parameters within their specified ranges
        """
        scaled_parameters = torch.zeros_like(predicted_parameters)
        for i, (min_val, max_val) in enumerate(ranges):
            scaled_parameters[i] = (
                predicted_parameters[i] * (max_val - min_val) + min_val
            )
    ...
    def epoch(self):
        ...
        for batch_no, batch_idx in enumerate(self.batches[:-1]):
            ...
            predicted_parameters = self.neural_net(batch_data)  # (#_to_learn)

            # Ensure the predicted parameters are non-zero
            temp = 1
            while (
                torch.any(predicted_parameters == 0)
                or torch.isnan(predicted_parameters).any()
            ):
                log.info(
                    f"Predicted parameters contain zero-value: {predicted_parameters}"
                )
                batch_data = self.training_data[batch_idx + temp, 0, :]  # (1, 1)
                predicted_parameters = self.neural_net(batch_data)  # (#_to_learn)
                temp += 1

            # Scale the learned params & store them in a dictionary
            parameters = {
                p: (
            ...
            t_true = torch.arange(
                0, self.batches[batch_no + 1] + 1, dtype=torch.float32
            ).to(self.device)

            # Solve ODE system
            solution = odeint(
                func=lambda y, t: self.apolo_model_7(y, t, params),
                y0=y0,
                t=t_true,
            ...
            pred_densities = torch.abs(solution[1:])  # (batch_size, #_latent_var)
            obser_densities = self.training_data[
                self.batches[batch_no] : self.batches[batch_no + 1], :, :
            ].squeeze()  # (batch_size, #_latent_var, 1)

            # Compute loss
            loss = self.loss_function(
                pred_densities[:, 0].squeeze(), obser_densities[:, 0].squeeze()
                ).sum()
            ...
            if self.grad_clip:
                torch.nn.utils.clip_grad_value_(
                    self.neural_net.parameters(), self.grad_clip
                )
        ...
        # Log the predictions and observations to a CSV file
        if self._time == self.num_epochs - 1:
            self.save_predictions_to_csv(
                # Rescale back to original range
                torch.expm1(pred_densities[:, 0].squeeze()), self.training_data_org[:, 0].squeeze(),
                self._h5group.file.filename.split(".")[0] + "_pred_obser.csv"
            )

    def save_model_weights(self, filepath):
        """Save the neural network model weights to a file.

        Args:
            filepath (str): Path where to save the model weights
                          (should end with .pth or .pt)
        """
        try:
            ...
            log.info(f"Model weights saved successfully to {filepath}")

        except Exception as e:
            log.error(f"Error saving model weights: {str(e)}")
            raise
    ...
    def save_predictions_to_csv(self, pred_densities, obser_densities, filepath):
        """
        Save predicted and observed densities to a CSV file.

        Args:
            pred_densities (torch.Tensor or np.ndarray): Predicted densities.
            obser_densities (torch.Tensor or np.ndarray): Observed densities.
            filepath (str): Path to save the CSV file.
        """
        # Convert to numpy if torch tensor
        if isinstance(pred_densities, torch.Tensor):
            pred_densities = pred_densities.detach().cpu().numpy()
        if isinstance(obser_densities, torch.Tensor):
            obser_densities = obser_densities.detach().cpu().numpy()

        df = pd.DataFrame({
            "ground_truth": obser_densities.flatten(),
            "nn_prediction": pred_densities.flatten(),
        })
