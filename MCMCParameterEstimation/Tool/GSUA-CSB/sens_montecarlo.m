...
set_param(model, 'StartTime',num2str(domain(1)));
set_param(model, 'StopTime',num2str(domain(2)));
...
if parallel
    ...
    try
        out = sim(in, 'ShowSimulationManager', sim_progress,'ShowProgress','off','UseFastRestart','off');
    catch
        switch sim_progress
            case 'on'
            disp('SimulationManager has not been detected');
        end
        out = sim(in,'ShowProgress',sim_progress,'UseFastRestart','off');
    end
end
failed=0;
lenout=length(output);
if N>1 || (init(1)==0 && N==1)
    parfor (i = 1:N,parforArg)
        try
            ytemp = interp1(out(i).get('tout'),out(i).get('yout'),xdata')'; % Y is a matrix of NxNd
            for j=1:lenout
                Y(i,:,j)=ytemp(output(j),:);
            end
        catch
            failed=failed+1;
        end
