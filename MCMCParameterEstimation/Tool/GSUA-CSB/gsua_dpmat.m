...
tPars=strsplit(tPars(1:end-1),{', ','[','matrix(',']'},'CollapseDelimiters',true);
tPars=tPars(~cellfun(@isempty, tPars));
tPars=tPars';
...
if size(domain,2)==1
    ...
    Creator.odes = odes;
    Creator.vars = vars;
    Creator.modelname = model_name;

try
    Table=addprop(Table,{'Solver' 'NumVars' 'output' 'Domain' 'rMethod' 'Kind' 'Fixed' 'Vars','Sname', 'creator'},...
    ...
    if isempty(output)
        Table.Properties.CustomProperties.output=1:(Table.Properties.CustomProperties.NumVars+limit);
    else
